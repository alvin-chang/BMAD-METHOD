...
memory_integration:
  ...
  startup_search: |
    search_memory("PERFORMANCE_AUDIT PERFORMANCE_LOAD PERFORMANCE_OPT PERFORMANCE_CAPACITY PERFORMANCE_MONITOR performance optimization bottleneck", project_id="{project_name}")
...
enhanced_workflow:
  before_performance_work:
    1. "search_memory('PERFORMANCE_AUDIT PERFORMANCE_LOAD PERFORMANCE_OPT performance optimization bottleneck', project_id='{project_name}')"
    2. "search_memory('ARCH_DECISION ARCH_TECH DEVOPS_INFRA architecture infrastructure', project_id='{project_name}')"
    3. "search_memory('DEV_CODE QA_NFR development testing nfr', project_id='{project_name}')"
    4. Review existing performance assessments and optimization patterns to avoid conflicts
    5. Identify gaps in current performance coverage

  during_performance_work:
    1. "store_memory('PERFORMANCE_AUDIT: [performance audit findings and optimization recommendations]', project_id='{project_name}')"
    2. "store_memory('PERFORMANCE_LOAD: [load test results and capacity analysis]', project_id='{project_name}')"
    3. "store_memory('PERFORMANCE_OPT: [optimization strategies and implementation plans]', project_id='{project_name}')"
    4. "store_memory('PERFORMANCE_CAPACITY: [capacity planning and scaling recommendations]', project_id='{project_name}')"
    5. "store_memory('PERFORMANCE_MONITOR: [monitoring setup and alerting configuration]', project_id='{project_name}')"
    6. "store_memory('PERFORMANCE_BOTTLENECK: [bottleneck analysis and remediation plans]', project_id='{project_name}')"
    7. "store_memory('PERFORMANCE_SCALE: [scalability test results and scaling strategies]', project_id='{project_name}')"
    8. "store_memory('PERFORMANCE_BENCHMARK: [performance benchmarks and baseline metrics]', project_id='{project_name}')"

  handoff_preparation:
    1. "search_memory('PERFORMANCE_AUDIT PERFORMANCE_LOAD PERFORMANCE_OPT all performance outputs', project_id='{project_name}')"
    2. Summarize all performance findings and optimization recommendations for development and operations teams
    3. "store_memory('PERFORMANCE_HANDOFF: [summary for development and operations teams]', project_id='{project_name}')"
...
memory_rules:
  ...
  3. "ALWAYS use proper PERFORMANCE_ prefixes when storing performance decisions"
...
command_examples:
  memory_aware_performance_audit: |
    *performance-audit {component_name}
    # Will automatically:
    # 1. search_memory("PERFORMANCE_AUDIT PERFORMANCE_LOAD ARCH_TECH DEVOPS_INFRA performance optimization", project_id="{project_name}")
    # 2. Review existing performance assessments before creating new audit procedures
    # 3. store_memory("PERFORMANCE_AUDIT: {audit_findings}", project_id="{project_name}")

  context_aware_load_test: |
    *load-test {target_name}
    # Will automatically:
    # 1. search_memory("PERFORMANCE_LOAD PERFORMANCE_CAPACITY ARCH_TECH existing load tests", project_id="{project_name}")
    # 2. search_memory("DEV_CODE ARCH_DECISION application architecture", project_id="{project_name}")
    # 3. Build on existing load test patterns and avoid duplicate testing
    # 4. store_memory("PERFORMANCE_LOAD: {load_test_results}", project_id="{project_name}")

  optimization_analysis: |
    *optimization-analysis {system_name}
    # Will automatically:
    # 1. search_memory("PERFORMANCE_OPT PERFORMANCE_AUDIT PERFORMANCE_BOTTLENECK optimization analysis", project_id="{project_name}")
    # 2. search_memory("ARCH_DECISION DEV_CODE system architecture", project_id="{project_name}")
    # 3. Apply consistent optimization standards based on project patterns
    # 4. store_memory("PERFORMANCE_OPT: {optimization_plan}", project_id="{project_name}")
